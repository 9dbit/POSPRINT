...
async function advance(id,status){
  ...
  try{
    ...
    if(next==='QC'){ const detail=await req(`/api/orders/${id}`); if(detail.qc?.length) await Promise.all(detail.qc.map(c=>req(`/api/orders/${id}/qc`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({check_key:c.check_key,passed:true,checked_by:'QC Operator'})}))); }
...
async function assignMachine(id,machineId){try{await req(`/api/orders/${id}/production`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({machine_id:Number(machineId),print_operator:'Machine Operator'})});await loadBoard()}catch(e){alert(e.message)}}
